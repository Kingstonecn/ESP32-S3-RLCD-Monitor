...
esp_err_t usage_client_fetch(const char *url, const char *token, usage_report_t *out)
{
    ...
    if (cJSON_IsObject(ds)) {
        ...
        const cJSON *tkc = cJSON_GetObjectItemCaseSensitive(ds, "today_cost_cny");
        const cJSON *tkp = cJSON_GetObjectItemCaseSensitive(ds, "today_cache_pct");
        const cJSON *tl  = cJSON_GetObjectItemCaseSensitive(ds, "today_label");
        const cJSON *mt  = cJSON_GetObjectItemCaseSensitive(ds, "month_tokens");
        const cJSON *mc  = cJSON_GetObjectItemCaseSensitive(ds, "month_cost_cny");
        const cJSON *mp  = cJSON_GetObjectItemCaseSensitive(ds, "month_cache_pct");
        ...
        out->deepseek.today_cost_cny  = cJSON_IsNumber(tkc) ? tkc->valuedouble : 0.0;
        out->deepseek.today_cache_pct = cJSON_IsNumber(tkp) ? (float) tkp->valuedouble : -1.0f;
        if (cJSON_IsString(tl)) strncpy(out->deepseek.today_label, tl->valuestring, sizeof(out->deepseek.today_label) - 1);
        else strncpy(out->deepseek.today_label, "TODAY", sizeof(out->deepseek.today_label) - 1);
        out->deepseek.month_tokens    = cJSON_IsNumber(mt)  ? (int64_t) mt->valuedouble : 0;
        out->deepseek.month_cost_cny  = cJSON_IsNumber(mc)  ? mc->valuedouble : 0.0;
        out->deepseek.month_cache_pct = cJSON_IsNumber(mp)  ? (float) mp->valuedouble : -1.0f;
